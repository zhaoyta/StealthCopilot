...
[![StealthCopilot 实机演示](docs/assets/screenshots/dashboard-running.png)](https://www.bilibili.com/video/BV1PHjm6HE75/)
...
| 初始化向导 | 面试主控台 |
|---|---|
| ![初始化向导](docs/assets/screenshots/setup-home.png) | ![面试主控台](docs/assets/screenshots/dashboard-running.png) |

| 服务密钥配置 | 简历 RAG 管理 |
|---|---|
| ![服务密钥配置](docs/assets/screenshots/settings-api.png) | ![简历 RAG 管理](docs/assets/screenshots/settings-resume.png) |

| 私有提词窗 | 双链路运行状态 |
|---|---|
| ![私有提词窗](docs/assets/screenshots/teleprompter-live.png) | ![双链路运行状态](docs/assets/screenshots/dashboard-live.png) |

---

## 一句话介绍

StealthCopilot 是一个运行在 macOS / Windows 上的开源桌面面试辅助系统，通过两条实时链路和一个可选视频输出模式完成：

- **听力链**：面试官语音 -> RTASR 实时转写 -> 机器翻译字幕 / 耳机译文播报 -> 意图识别 -> 简历 RAG -> AI 回答建议
- **说话链**：你的母语语音 -> 语音翻译 -> 可选润色 -> 默认音色或个人复刻音色 TTS -> 虚拟麦克风
- **数字人输出模式**：作为说话链的可选输出，将 TTS 音频发送给 Simli AI 生成口型同步视频，并通过 OBS Browser Source + OBS Virtual Camera 输出到会议摄像头；ZEGO 数字人保留为企业级可选 Provider
...
| 数字人视频输出 | 通过 Simli AI 生成口型同步画面；App 在本机提供 OBS 浏览器源，OBS 负责输出到 `OBS Virtual Camera`。 |
...
flowchart LR
  ...
  subgraph Local["StealthCopilot Desktop"]
    ...
    D1["OBS browser source"]
    D2["OBS scene"]
    D3["OBS Virtual Camera"]
    E1["Config + Keychain"]
    E2["Circuit Breaker"]
  end
...
- TTS 音频按 PCM 时钟节拍写入虚拟麦克风；启用 Simli 数字人时，同一段 PCM 会同时发送给 Simli 生成口型同步视频。
...
- 数字人不是独立业务链，而是说话链的可选输出模式；说话链仍负责翻译、TTS 和虚拟麦克风输出。
- Simli 当前提供视频口型同步；音频仍由本地 TTS 写入虚拟麦克风。为了抵消 Simli/OBS/会议软件的视频延迟，本地音频默认延迟约 700ms 写入虚拟麦克风。
- App 不再自研或注册虚拟摄像头驱动；必须打开 OBS App，在场景中添加浏览器源 `http://127.0.0.1:18765/`，并启动 `OBS Virtual Camera`。
- OBS 浏览器源页面由 App 启动说话链时提供；若说话链未启动，OBS 可能黑屏。

### 4. 熔断与降级

面试场景里，最坏的体验不是“AI 不够聪明”，而是会议中断。因此 StealthCopilot 把降级作为一等能力：

- 数字人视频不可用时，停止当前数字人输出并提示用户；说话链可关闭数字人后继续只输出虚拟麦克风音频。
...
| 数字人视频输出 | 没有 | 没有 | 部分支持 | Simli + OBS Virtual Camera |
...
首次启动后进入设置向导，先配置启动核心链路所需的凭证；可选能力可稍后在设置页补充：
...
| Simli AI / ZEGO 数字人 | 数字人视频口型同步 / 企业级数字人输出 | 可选；在设置页补充，不配置时说话链只输出虚拟麦克风音频 |
...
- 麦克风：系统配置的虚拟声卡，例如 `BlackHole 2ch`
- 摄像头：`OBS Virtual Camera`

如果启用数字人视频输出，还需要先在 OBS 中添加浏览器源：

```text
http://127.0.0.1:18765/
```

OBS App 必须保持打开，并点击「启动虚拟摄像机」。会议软件看不到 `OBS Virtual Camera` 时，优先确认 OBS 虚拟摄像机已启动；macOS 上若 OBS Camera Extension 刚安装或更新，可能需要重启系统。
...
- 听力链：实时字幕和回答建议
- 说话链：母语语音转目标语言语音；启用数字人时同步输出所选 Provider 的数字人视频

也可以使用“一键启动全部”按顺序启动听力链和说话链。
...
| 配置区域 | 内容 |
|---|---|
| 服务密钥 | 讯飞 RTASR、讯飞机器翻译、讯飞声音复刻、DeepSeek、Simli AI、ZEGO 数字人连接信息与连通性测试 |
| 语言 | 听力链源语言/目标语言，说话链输入语言/输出语言 |
| 设备 | 虚拟声卡、物理麦克风、OBS 虚拟摄像头、数字人服务商与浏览器源地址 |
...
| 数字人视频 | Provider 化，默认 Simli AI；ZEGO 作为企业级可选 Provider；本机输出为 OBS Browser Source + OBS Virtual Camera |
...
项目已具备完整桌面应用骨架、听力链、说话链、数字人输出模式、设置向导、提词窗、配置存储、RAG 本地索引、CI、lint 和测试入口。

生产部署前建议完成：

- 做真实面试设备联调：耳机译文播报支持选择监听输出设备；macOS 会优先通过 ffmpeg AudioToolbox 路由到指定输出，其他平台或缺少 ffmpeg 时回退到系统默认输出。
- 使用真实讯飞 / DeepSeek / 讯飞声音复刻 / Simli / ZEGO / OBS 做端到端联调。
- 在 macOS 和 Windows 实机验证虚拟麦克风、OBS Virtual Camera、会议软件设备选择。
...
| Digital Human Output | Simli Provider、OBS Browser Source、OBS Virtual Camera、音视频延迟校准 |
...
- [数字人与 OBS 配置](docs/guide/digital-human.md)
