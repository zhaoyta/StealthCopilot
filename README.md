...
| 能力 | 说明 |
|---|---|
| 实时听译字幕 | 捕获会议音频，通过讯飞 RTASR 实时语音转写输出源语言文本，最终句子再调用讯飞机器翻译文本接口输出目标语言文本。 |
...
| 母语到目标语言语音 | 本地 VAD 检测用户说话结束，使用讯飞 RTASR 得到母语文本，再由讯飞机器翻译文本接口/润色链得到目标文本，最后由讯飞声音复刻流式 TTS 输出真实人声。 |
...
| 服务 | 用途 | 是否必需 |
|---|---|---|
| 讯飞开放平台 | RTASR 实时语音转写 + 机器翻译文本接口 | 听力/说话链必需 |
| DeepSeek | 意图识别、回答生成、可选润色 | 回答建议必需 |
| 讯飞声音复刻 | 克隆音色 TTS | 说话链语音输出必需 |
...
| 配置区域 | 内容 |
|---|---|
| 服务密钥 | 讯飞、DeepSeek、讯飞声音复刻、Simli AI 连接信息与连通性测试 |
...
| 安全存储 | go-keyring |
| ASR/文本翻译 | Provider 化，默认讯飞 RTASR 实时语音转写 + 讯飞机器翻译文本接口（v1 优先，403 not found 时降级 v2） |
| LLM | OpenAI-compatible Provider，默认 DeepSeek |
| TTS | Provider 化，默认讯飞声音复刻 |
