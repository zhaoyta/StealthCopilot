...
提供 7 Tab 的设置面板 UI，涵盖 API 凭证、语言配置、设备绑定、简历管理、历史会话、提词窗外观和高级配置，所有改动实时写入 Keychain 或本地配置，无需手动保存。

---

## Requirements

### Requirement: 7 Tab 设置面板
设置面板 SHALL 包含 7 个 Tab：API 凭证、语言配置、设备绑定、简历管理、历史会话、提词窗外观、高级。Tab 切换无需保存操作，各 Tab 内改动实时写入 Keychain / 本地配置。
