...
### Requirement: 依赖检测与安装指引
Step 2 SHALL 检测 BlackHole 虚拟声卡、FFmpeg 和 OBS Virtual Camera 支持是否可用。缺失时提供安装或打开官方页面的指引，安装时显示进度条或外部说明。

#### Scenario: 依赖已安装
- **WHEN** 检测到 BlackHole、FFmpeg 和 OBS Virtual Camera 支持均可用
- **THEN** 各项均显示"已安装"绿色状态，可直接进入下一步

#### Scenario: 一键安装缺失依赖
- **WHEN** 用户点击缺失依赖的"一键安装"按钮
- **THEN** 对 BlackHole / FFmpeg 执行对应安装流程或打开官方安装页面；对 OBS 打开 OBS 官方下载页面，不注册自研虚拟摄像头驱动
...
- **THEN** 仍可进入下一步，数字人视频输出在设置补全前禁用；说话链虚拟麦克风音频仍可使用
