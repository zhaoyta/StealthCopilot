...
func NewClassifier(apiKey, model string) *Classifier {
	return NewClassifierWithConfig(llm.Config{APIKey: apiKey, Model: model})
}

func NewClassifierWithConfig(cfg llm.Config) *Classifier {
	return &Classifier{
		apiKey:  cfg.APIKey,
		model:   cfg.Model,
		baseURL: cfg.BaseURL,
...
func (c *Classifier) callDeepSeek(ctx context.Context, srcText string) (IntentType, error) {
	...
	httpReq, err := http.NewRequestWithContext(
		ctx,
		http.MethodPost,
		llm.Config{BaseURL: c.baseURL}.ChatCompletionsURL(),
